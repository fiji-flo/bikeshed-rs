...
pub fn correct_h1(dom: &NodeRef) {
    if let Ok(h1_els) = dom.select("h1") {
        ...
        if h1_els.len() >= 2 {
            html::node::replace_node(h1_els[0].as_node(), h1_els[1].as_node());
        }
    }
}
...
pub fn clean_dom(dom: &NodeRef) {
    ...
    if let Ok(dt_els) = dom.select("dt[data-md]") {
        for dt_el in dt_els {
            let child = match html::node::get_only_child(dt_el.as_node()) {
                Some(child) => child,
                None => continue,
            };

            let p_el = if html::node::get_tag(&child).unwrap() == "p" {
                child
            } else {
                continue;
            };

            html::node::copy_content(&p_el, dt_el.as_node());
            p_el.detach();
        }
    }
}
